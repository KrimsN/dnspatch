package beget

import "github.com/KrimsN/dnspatch/internal/httpx"

// Config holds the parameters of the beget provider.
type Config struct {
	Login    string `toml:"login" required:"true" doc:"Beget account login used for API calls"`
	Password string `toml:"password" required:"true" example:"${BEGET_PASSWORD}" doc:"Beget account password"`
	Zone     string `toml:"zone" required:"true" example:"example.com" doc:"Domain name of the zone, for example example.com"`
	RRName   string `toml:"rr_name" required:"true" example:"home" doc:"Record name relative to the zone: @ for the apex, * for a wildcard, or a label such as home"`
	BaseURL  string `toml:"base_url" default:"https://api.beget.com/api" doc:"Base URL of the API"`

	httpx.ProxyConfig
}
