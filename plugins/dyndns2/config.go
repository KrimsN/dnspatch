...
type Config struct {
	...
	IPv6Param string `toml:"ipv6_param" default:"ipv6" doc:"Query parameter that carries the IPv6 address; both addresses go in one request, so the service must know this parameter to update an AAAA record. A family the retrievers did not report is not sent, and the service may then fall back to the address the request came from"`
