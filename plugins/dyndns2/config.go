...
type Config struct {
	BaseURL   string `toml:"base_url,required" example:"https://api.nic.ru/dyndns/update" doc:"Update URL of the service, for example https://api.nic.ru/dyndns/update for NIC.RU or https://updates.dnsomatic.com/nic/update for DNS-O-Matic"`
	Username  string `toml:"username,required" doc:"Login for HTTP Basic authentication; for NIC.RU the login of the contract or of the account allowed to use Dynamic DNS"`
	Password  string `toml:"password,required,secret" example:"${PASSWORD}" doc:"Password for HTTP Basic authentication; some services issue a separate update password"`
	Hostname  string `toml:"hostname,required" example:"home.example.com" doc:"Full domain name of the record to update. NIC.RU changes the A records with this name in every zone of the contract"`
