...
func TestBuildSendsBothAddressesToTheService(t *testing.T) {
	...
	if q := got.URL.Query(); q.Get("hostname") != "home.example.com" || q.Get("myip") != "203.0.113.7" || q.Get("myipv6") != "2001:db8::7" {
		t.Errorf("query = %v", q)
	}
...
func TestEndpointIsNoIP(t *testing.T) {
	if endpoint != "https://dynupdate.no-ip.com/nic/update" {
