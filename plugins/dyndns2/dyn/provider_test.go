...
func TestBuildSendsBothAddressesToTheService(t *testing.T) {
	...
	if q := got.URL.Query(); q.Get("hostname") != "home.example.com" || q.Get("myip") != "203.0.113.7,2001:db8::7" || q.Has("myipv6") {
		t.Errorf("query = %v", q)
	}
...
func TestEndpointIsDyn(t *testing.T) {
	if endpoint != "https://members.dyndns.org/nic/update" {
