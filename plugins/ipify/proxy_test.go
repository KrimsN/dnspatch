...
func TestRetrieverIgnoresProxyEnvironmentByDefault(t *testing.T) {
	...
	base := Config{BaseURL: "https://api64.ipify.org", Family: "ipv4"}
...
func TestRetrieverThroughProxy(t *testing.T) {
	tests := []struct {
		...
		{name: "ipv4", family: "ipv4", reply: "203.0.113.7", want: "203.0.113.7"},
		// The connection to the proxy is IPv4 here; only the reply says v6.
		{name: "ipv6 is not pinned on the connection to the proxy", family: "ipv6", reply: "2001:db8::1", want: "2001:db8::1"},
	}
...
func TestRetrieverBadProxyFailsAtConstruction(t *testing.T) {
	cfg := withProxy(Config{BaseURL: "https://api64.ipify.org", Family: "ipv4"}, "ftp://u:"+proxyPass+"@proxy.example.com")
