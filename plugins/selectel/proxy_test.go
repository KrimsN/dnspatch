...
const (
	...
	apiHost = "api.selectel.test"
)

// proxiedConfig points a provider at the fake API through a SOCKS5 proxy that
// requires a login. Both the identity service and the DNS API sit behind the
// same fake host, since the provider does not distinguish the two when
// choosing whether to proxy.
func proxiedConfig(t *testing.T, srv *httptest.Server) (Config, *socks5test.Server) {
	...
	cfg := testConfig(srv)
	cfg.AuthURL = "http://" + apiHost + "/identity/v3"
	cfg.BaseURL = "http://" + apiHost + "/domains/v2"
...
func TestProviderUsesProxy(t *testing.T) {
	...
	if got := proxy.Targets(); len(got) == 0 {
		t.Error("no request went through the proxy")
