...
type Config struct {
	Server       string        `toml:"server" required:"true" example:"ns1.example.com:53" doc:"Address of the name server that accepts updates, as host or host:port. The port defaults to 53. It must be the primary (master) server of the zone or one that forwards updates to it"`
	Zone         string        `toml:"zone" required:"true" example:"example.com" doc:"Domain name of the zone the server is authoritative for, for example example.com"`
