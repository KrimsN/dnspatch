...
type Config struct {
	Key     string `toml:"key,required" example:"${YANDEX_CLOUD_KEY}" doc:"Authorized key of the service account as JSON, the file that yc iam key create writes. The account needs the dns.editor role on the folder of the zone"`
