...
type Config struct {
	Key     string `toml:"key" required:"true" example:"${YANDEX_CLOUD_KEY}" doc:"Authorized key of the service account as JSON, the file that yc iam key create writes. The account needs the dns.editor role on the folder of the zone"`
	ZoneID  string `toml:"zone_id" required:"true" example:"dns1234567890abcdefgh" doc:"ID of the DNS zone, shown in the Cloud DNS console or by yc dns zone list"`
